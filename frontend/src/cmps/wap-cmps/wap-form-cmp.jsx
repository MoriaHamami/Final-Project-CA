...
export function FormCmp({ style, cmp, onElClick }) {
    ...
    return (
        <div id={cmp.type} style={style} className={cmp.name} data-container={JSON.stringify(cmp)} onClick={onElClick}>
            <h2 style={info.title.style} data-container={JSON.stringify(info.title)} contentEditable={selectedElementId === info.title.id} onClick={onElClick} className="title">{info.title.txt}</h2>
            <span style={info.subtitle.style} data-container={JSON.stringify(info.subtitle)} contentEditable={selectedElementId === info.subtitle.id} onClick={onElClick} className="subtitle">{info.subtitle.txt}</span>
            <form>
                ...
                <button style={info.btn.style} data-container={JSON.stringify(info.btn)} contentEditable={selectedElementId === info.btn.id} onClick={onElClick} >{info.btn.label}</button>
            </form>
        </div>
    )
}
