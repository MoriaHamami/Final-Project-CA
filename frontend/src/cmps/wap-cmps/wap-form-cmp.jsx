...
export function FormCmp({ style, cmp, onElClick, selectedCmpId }) {
    ...
    return (
        <div id={cmp.type} style={style} className={cmp.name} data-container={JSON.stringify(cmp)} onClick={onElClick}>
            ...
            {info.txt && <span style={info.txt.style} data-container={JSON.stringify(info.txt)} contentEditable={selectedElement?.id === info.txt.id} onClick={onElClick} className="txt">{info.txt.txt}</span>}
            <form>
